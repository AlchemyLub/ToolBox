...
public class SettingsCodeGenTask : Task
{
    ...
    /// Директория проекта
    /// </summary>
    public required string ProjectDirectory { get; set; } = string.Empty;
    ...
    /// </summary>
    public string ClassName { get; set; } = "AppSettingsKeys";
    ...
    public override bool Execute()
    {
        ...
        try
        {
            string[] settingsFiles = Directory.GetFiles(ProjectDirectory, "appsettings*.json");
            ...
            string outFile = Path.Combine(ProjectDirectory, "Generated", $"{ClassName}.cs");
            if (!ShouldRegenerate(settingsFiles, outFile))
            {
            ...
            string generatedClass = GenerateSettingsClass(root, NamespaceName, ClassName);

            string outputPath = Path.Combine(ProjectDirectory, "Generated");
            Directory.CreateDirectory(outputPath);

            string outputFile = Path.Combine(outputPath, $"{ClassName}.cs");
            File.WriteAllText(outputFile, generatedClass);

            Log.LogMessage(MessageImportance.Normal, $"Generated {ClassName}.cs");
            return true;
        }
        catch (Exception ex)
        {
            Log.LogError($"Settings code generation failed: {ex.Message}");
            return false;
        }
    }
    ...
    private static string ToValidClassName(string name)
    {
        ...
        result = new(result.Where(c => char.IsLetterOrDigit(c) || c == '_').ToArray());
